...
function get_user_achievements_ajax_controller() {
	...
	$userAchievements = get_member_profile ( $userkey, $handle );
 ...
 */ 
/*
 * Get trach history stats
 * */
function get_track_history_data($handle='',$track=''){
	if($handle==null || $handle == ''){
		$handle = $_POST ["handle"];
		$track = $_POST ["track"];
	}
	$url = "http://api.topcoder.com/v2/develop/statistics/$handle/$track";
	$args = array (
			'httpversion' => get_option ( 'httpversion' ),
			'timeout' => get_option ( 'request_timeout' )
	);
...
function get_track_history_controller(){
	$historyData = get_track_history_data();
	echo $historyData;
	die();
}
add_action ( 'wp_ajax_get_track_history', 'get_track_history_controller' );
add_action ( 'wp_ajax_nopriv_get_track_history', 'get_track_history_controller' );